...
type BindPhoneVerifyScreenNavigationProp = NativeStackNavigationProp<ProfileStackParamList, 'BindPhoneVerify'>;

const CODE_LENGTH = 6;
const COUNTDOWN_SECONDS = 60;

const BindPhoneVerifyScreen: React.FC = ({type,email,phoneNumber,countryCode,actionToken}:any) => {
  const navigation = useNavigation<BindPhoneVerifyScreenNavigationProp>();
  const { t } = useLanguage();
  ...
  const handleResend = async () => {
    if (countdown === 0 && !isResending) {
      ...
      try {
        ...
        if (type === 'email') {
          // 发送邮箱验证码
          response = await sendVerificationCodeToNew({
          ...
          response = await sendVerificationCodeToNew({
            action_token: actionToken,
            new_identifier: `${countryCode}${phoneNumber}`,
  ...
  const handleSubmit = async () => {
    navigation.navigate('BindSuccess', {
      bindType: 'email',
      bindValue: email,
    });
            ...
            <View style={styles.resendContainer}>
              ...
              >
                <Text style={[
                  ...
                  Get verification code again  {countdown > 0&&countdown+"s"}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
...
export default BindPhoneVerifyScreen; 